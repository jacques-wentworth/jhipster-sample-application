...
export class TaskService {
  ...
  protected convertDateFromClient(task: ITask): ITask {
    const copy: ITask = Object.assign({}, task, {
      test1: task.test1 && task.test1.isValid() ? task.test1.toJSON() : undefined,
      test2: task.test2 && task.test2.isValid() ? task.test2.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.test1 = res.body.test1 ? moment(res.body.test1) : undefined;
      res.body.test2 = res.body.test2 ? moment(res.body.test2) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((task: ITask) => {
        task.test1 = task.test1 ? moment(task.test1) : undefined;
        task.test2 = task.test2 ? moment(task.test2) : undefined;
      });
    }
    return res;
  }
